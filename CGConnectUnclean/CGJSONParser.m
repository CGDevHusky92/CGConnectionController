...
@property (strong, nonatomic) NSMutableDictionary * arrayMap;
@property (strong, nonatomic) NSMutableDictionary * propertyClasses;
...
{
    if (!self.arrayMap) self.arrayMap = [[NSMutableDictionary alloc] init];
...
- (id)process:(id)object error:(NSError *__autoreleasing *)error
{
    ...
    if ([object isKindOfClass:[NSDictionary class]]) {
        ...
        NSMutableArray * propArray = [[self getPropertiesOfClass:self.objClass] mutableCopy];
        
        [propArray addObjectsFromArray:[NSArray arrayWithObjects:@"createdAt", @"updatedAt", @"note", @"wasDeleted", @"syncStatus", nil]];
        
#warning Try adding properties if NSManagedObject???
        
        id newObject = nil;
        if ([[self.objClass class] respondsToSelector:@selector(newModel)]) {
            newObject = objc_msgSend([self.objClass class], @selector(newModel));//[[self.objClass class] performSelector:@selector(newModel)];
        } else {
            if ([self.objClass isSubclassOfClass:[NSManagedObject class]]) {
                newObject = [[CGDataController sharedData] newManagedObjectForClass:NSStringFromClass([self.objClass class])];
            } else {
        ...
        for (NSString * propName in propArray) {
            if ([propName isEqualToString:@"objectId"]) {
                if ([self assignValue:@"id" propName:propName dict:dict obj:newObject error:error]) continue;
            }
            
            if ([propName isEqualToString:@"objDescription"]) //special edge case for objective-c using the description method
            {
                if ([self assignValue:@"description" propName:propName dict:dict obj:newObject error:error]) continue;
            }
            
            if ([self assignValue:propName propName:propName dict:dict obj:newObject error:error]) continue;
            
            NSString * objCName = [CGJSONParser convertToJsonName:propName];
...
- (BOOL)assignValue:(NSString *)key propName:(NSString *)propName dict:(NSDictionary *)dict obj:(id)obj error:(NSError **)error
{
    ...
    if (value) {
        if ([[NSDate class] isSubclassOfClass:self.propertyClasses[propName]]) {
            ...
            if (date) {
                NSLog(@"Detected Date and Setting - %@", date);
        ...
        if ([value isKindOfClass:[NSDictionary class]] && ![[obj valueForKey:propName] isKindOfClass:[NSDictionary class]]) {
            ...
            id dic = [self.propertyClasses[propName] objectWithParser:value error:&childError];
            ...
            if (dic) {
                [obj setValue:dic forKey:propName];
                return YES;
            }
        }
        ...
        if ([value isKindOfClass:[NSArray class]]) {
            ...
            if (array.count > 0 && [array[0] isKindOfClass:[NSDictionary class]] && arrayClass) {
                ...
                for (NSDictionary * dict in array) {
                    NSError * childError = nil;
                    id joy = [[CGJSONParser JSONParserWithClass:arrayClass] process:dict error:&childError];
                    if (joy && !childError)
        ...
        if (self.propertyClasses[propName] && ![value isKindOfClass:self.propertyClasses[propName]] && [NSNull null] != (NSNull *)value) {
            ...
            if ([value isKindOfClass:NSClassFromString(@"__NSCFBoolean")] && !boxDisabled) {
                if ([self.propertyClasses[propName] isSubclassOfClass:[NSString class]]) {
                    if ([NSNull null] == (NSNull *)value) {
                        NSLog(@"Setting Nil for %@", propName);
                        [obj setValue:nil forKey:propName];
                    } else {
                        NSLog(@"Setting %@ for %@", [NSString stringWithFormat:@"%@", value], propName);
            ...
            if (error) *error = [CGJSONParser errorWithDetail:errorString code:CGJSONParserErrorIncorrectType];
            return NO;
        }
...
- (NSArray *)getPropertiesOfClass:(Class)objectClass
{
    ...
    for (i = 0; i < outCount; i++) {
        ...
        if ([typeAttribute hasPrefix:@"T@"] && [typeAttribute length] > 3) {
            ...
            if (!self.propertyClasses) self.propertyClasses = [[NSMutableDictionary alloc] init];
...
{
    if ([NSNull null] == (NSNull *)dateString) return nil;
...
+ (instancetype)JSONParserWithClass:(Class)class
{
    return [[CGJSONParser alloc] initWithClass:class];
}

+ (NSError *)errorWithDetail:(NSString *)detail code:(CGJSONParserError)code
{
    NSMutableDictionary * details = [NSMutableDictionary dictionary];
    [details setValue:detail forKey:NSLocalizedDescriptionKey];
    return [[NSError alloc] initWithDomain:NSLocalizedString(@"CGJSONParser", nil) code:code userInfo:details];
}
...
@implementation NSObject (CGJSONParser)

+ (CGJSONParser *)jsonMapper
{
    return [[CGJSONParser alloc] initWithClass:[self class]];
}

+ (id)objectWithParser:(id)jsonObj
{
    if (!jsonObj) {
        NSLog(@"Error JSON Obj was nil");
        return nil;
    }
    return [self objectWithParser:jsonObj error:nil];
}

+ (id)objectWithParser:(id)jsonObj error:(NSError *__autoreleasing *)error
{
    CGJSONParser * mapper = [self jsonMapper];
